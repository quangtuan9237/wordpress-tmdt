 ...
 * Description: Accept payments from your WooCommerce store via Credit/Debit Cards, Apple Pay, Google Pay, Alipay, SEPA Pay, Klarna, Afterpay, WeChat Pay and Stripe Checkout using Stripe.
 * Author: WebToffee
...
if (!defined('EH_STRIPE_VERSION')) {
    define('EH_STRIPE_VERSION', '3.6.6');
}
...
if(is_plugin_active('eh-stripe-payment-gateway/stripe-payment-gateway.php')){ 
    ...
    function eh_stripe_overview_get_order_ids() {
        ...
        for ($i = 0, $count = 0; $i < count($id); $i++) {
            if (('eh_wechat_stripe' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_afterpay_stripe' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_klarna_stripe' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_sepa_stripe' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_stripe_pay' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_alipay_stripe' === get_post_meta($id[$i], '_payment_method', true)) || ('eh_stripe_checkout' === get_post_meta($id[$i], '_payment_method', true))) {
