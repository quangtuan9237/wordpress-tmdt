...
class EH_Stripe_Payment extends WC_Payment_Gateway {
    ...
    public function make_charge_params($charge_value, $order_id) {
        ...
        $charge_parsed = array(
            ...
            "source_type" => ('card' == $charge_data->payment_method_details->type ) ? ($charge_data->payment_method_details->card->brand . "( " . $charge_data->payment_method_details->card->funding . " )") : (( 'alipay' == $charge_data->payment_method_details->type ) ? 'Alipay' : (( 'sepa_debit' == $charge_data->payment_method_details->type ) ? 'Sepa Debit' : (('klarna' == $charge_data->payment_method_details->type ) ? 'Klarna' : (('afterpay_clearpay' == $charge_data->payment_method_details->type ) ? 'Afterpay' : (('wechat' == $charge_data->payment_method_details->type ) ? 'Wechat' : 'Undefined'))))),
