...
class EH_Alipay_Stripe_Gateway extends WC_Payment_Gateway {
    ...
    public function get_payment_intent_from_order( $order ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            $intent_id = get_post_meta( $order_id, 'eh_alipay_intent_id', true );
        } else {
            $intent_id = $order->get_meta( 'eh_alipay_intent_id' );
        }
    ...
    public function save_payment_intent_to_order( $order, $intent ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            update_post_meta( $order_id, 'eh_alipay_intent_id', $intent->id );
        } else {
            $order->update_meta_data( 'eh_alipay_intent_id', $intent->id );
        }
	...
	public function process_payment( $order_id, $retry = true, $force_save_save = false ) {
        ...
        add_post_meta( $order_id, '_eh_alipay_payment_intent', $intent->id); 
