...
class EH_Sepa_Stripe_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = sprintf( __( 'SEPA (Single Euro Payments Area) Direct Debit payment authenticates customers using the IBAN number (supports %s EU %s). ' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/sepa-preview.png?TB_iframe=true&width=100&height=100"> [Preview] </a>
         
             <p> <a target="_blank" href="https://www.webtoffee.com/woocommerce-stripe-payment-gateway-plugin-user-guide/#sepa_pay"> Read documentation </a></p>', 'payment-gateway-stripe-and-woocommerce-integration' ), '<b>', '</b>' );
        $this->supports = array(
            'products',
            'refunds',
                       ...
                       if (isset($decoded['data']['object']['metadata']['order_id']) && !empty($decoded['data']['object']['metadata']['order_id'])) {
                            ...
                            if(!$order = wc_get_order( $order_id )){
                                ...
                                if (!empty($meta) && isset($meta[0]->post_id)) {
                                    ...
                                    $meta = $wpdb->get_results( "SELECT post_id FROM " . $wpdb->postmeta ." WHERE meta_key = 'eh_wechat_intent_id' AND meta_value= '" . $payment_intent_id  . "'" );
