...
class EH_Wechat extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description =  __( 'Accepts payments via WeChat Pay, a digital wallet popular with customers from China.' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/wechat-preview.png?TB_iframe=true&width=100&height=100"> [Preview] </a>
             <p> <a target="_blank" href="https://www.webtoffee.com/woocommerce-stripe-payment-gateway-plugin-user-guide/#wechat"> Read documentation </a></p>', 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->supports = array(
            'products',
            'refunds',
        );
    ...
    public function save_payment_intent_to_order( $order, $intent ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            update_post_meta( $order_id, 'eh_wechat_intent_id', $intent->id );
        } else {
            $order->update_meta_data( 'eh_wechat_intent_id', $intent->id );
        }
    ...
    public function get_payment_intent_from_order( $order ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            $intent_id = get_post_meta( $order_id, 'eh_wechat_intent_id', true );
        } else {
            $intent_id = $order->get_meta( 'eh_wechat_intent_id' );
        }
